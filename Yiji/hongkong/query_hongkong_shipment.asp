...
<!--  <td width="700" height="21" align="right"><font size=4><b>查询船期表</b></font></td> -->
	<td align="right">
	  搜索：
		...
		<%
			if fla22="0" then
				sql="select * from sales where name ='"&session("redboy_username")&"'"
			else
...
<%
  ...
  rs_shipment.open sql,conn,1,3
	...
	do while rs_shipment.eof=false
	  nowtotal = nowtotal + rs_shipment("actprofit")	
...
<td width="100%" background="../images/r_0.gif">
  <table cellpadding="0" cellspacing="0" width="100%">
    <tr>
      <td>&nbsp;香港船期表查询</td>
	  <td align="right">总利润：<b><%=formatnumber(nowtotal,3)%> 港币&nbsp;<a href="open_excel.asp?page=<%=currentpage%>&startdate=<%=nowstartdate%>&enddate=<%=nowenddate%>&customer=<%=nowcustomer%>&sales=<%=nowsales%>&keyword=<%=nowkeyword%>&order1=<%=request("order1")%>&order2=<%=request("order2")%>&order3=<%=request("order3")%>&order4=<%=request("order4")%>&order5=<%=request("order5")%>&order6=<%=request("order6")%>&order7=<%=request("order7")%>&order8=<%=request("order8")%>&order9=<%=request("order9")%>&order10=<%=request("order10")%>&order11=<%=request("order11")%>&order12=<%=request("order12")%>" target="_blank"><img src="../images/excel.jpg" border="0" align="absmiddle" alt="导出Excel表格"></a></td>
    </tr>
  </table>
</td>
