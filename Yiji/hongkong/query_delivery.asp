...
<%
  if nowkeyword<>"" then
  	sql="select * from delivery where delivery like '%"&nowkeyword&"%' order by delivery ASC"
	else
		sql="select * from delivery order by delivery ASC"
  end if
%>
...
<form name="form2" action="produit_del.asp">
  ...
  <tr align="center">
	<td class="category" width="200" height="30">货运公司</td>
  ...
  <%
  set rs_delivery =server.createobject("ADODB.RecordSet")	
  rs_delivery.open sql,conn,1,3
  if not rs_delivery.eof then
  do while rs_delivery.eof=false
  %>
  <tr align="center">
	<td align="center" height="30"><%=rs_delivery("delivery")%></td>
  <td align="center"><%=rs_delivery("description")%></td>	  
	<%if fla14="1" or session("redboy_id")="1" then%>
    <td align="center">
    	<a href="change_delivery.asp?form=<%=request("form")%>&delivery=<%=rs_delivery("delivery")%>&desc=<%=rs_delivery("description")%>&keyword=<%=nowkeyword%>"><img src="../images/res.gif" border="0" hspace="2" align="absmiddle">修改</a></td>
	<%end if%>
  </tr>
  <%
  	'end if
  ...
  %>
  <%
  if rs_delivery.recordcount>0 then 
  %> 
</table></td></tr>
<%end if%> 
</form>   
