...
<table align="center" cellpadding="4" cellspacing="1" class="toptable grid" border="1">
      <tr>
        ...
        <td class="category"><input type="text" name="delivery" style="width:150px"> 
...
sql="select * from delivery where delivery='"&nowdelivery&"'"
set rs=conn.execute(sql)
if rs.eof=false then
%>
<script language="javascript">
alert("您输入的货运公司已经存在，请重新输入！")
window.history.go(-1)
</script> 
<%
  response.end
...
sql="insert into delivery(delivery,Description,CreateDate,Creator) values('"&nowdelivery&"','"&nowdes&"',#"&now()&"#,'"&session("redboy_username")&"')"
conn.execute(sql)
%>
<script language="javascript">
alert("货运公司添加成功！")
