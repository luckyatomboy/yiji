...
<form name="form2">
  ...
  <tr> 
	...
	<td width="*" align="right">
	  搜索：
	  <select name="class" onChange="form2.submit()">
	    <option value="1"<%if nowclass=1 then%> selected="selected"<%end if%>>公司赠品</option>
		<option value="0"<%if nowclass=0 then%> selected="selected"<%end if%>>正常产品</option>
	  </select>
    <%
...
<%
  sql="select * from produit where class="&nowclass
  if nowkeyword<>"" then
  ...
  elseif request("order7")<>"" then
    sql=sql&" order by duihuan "&request("order7")
...
<table align="center" cellpadding="4" cellspacing="1" class="toptable grid" border="1">
  <tr align="center">    
	...
	<td class="category">
	  <a href="?order7=<%if request("order7")="asc" then%>desc<%else%>asc<%end if%>&form=<%=request("form")%>&field=<%=request("field")%>&field2=<%=request("field2")%>&page=<%=currentpage%>&keyword=<%=nowkeyword%>&ku=<%=nowku%>&bigclass=<%=nowbigclass%>&smallclass=<%=nowsmallclass%>" class="title">兑换积分<%if request("order7")="asc" then%><img src="../images/up2.gif" border="0" hspace="2" align="absmiddle"><%else%><img src="../images/down2.gif" border="0" hspace="2" align="absmiddle"><%end if%></a>	
	</td>
  </tr>
  <%
  ...
  %>
  <tr onMouseOver="this.className='highlight'" onMouseOut="this.className=''" <%if request("form")<>"" then%>onDblClick="window.opener.document.<%=request("form")%>.<%=request("field")%>.value='<%=rs_produit("huohao")%>';<%if request("field2")<>"" then%>window.opener.document.<%=request("form")%>.<%=request("field2")%>.value='<%=rs_produit("duihuan")%>';<%end if%><%if request("span1")<>"" then%>window.opener.document.all.<%=request("span1")%>.innerHTML='目前所有仓库数量：<%=nowshulian%>，<%=left(nowtext,len(nowtext)-1)%>';<%end if%>window.close();"<%else%>onDblClick="javascript:var win=window.open('produit_show.asp?id=<%=rs_produit("id")%>','产品详细信息','width=895,height=503,top=142,left=115,toolbar=no,location=no,directories=no,status=no,menubar=no,resizable=no,scrollbars=yes'); win.focus()"<%end if%>>
	...
	<td align="center"><%=rs_produit("duihuan")%></td>
  </tr>
  <%
  	'end if
    rs_produit.movenext
  ...
  <tr>
	...
	<table cellpadding=0 cellspacing=0 width="100%">
	<tr>
	<td width="20%" align="left" style="color:#FF0000;">&nbsp;双击选择产品</td>  
