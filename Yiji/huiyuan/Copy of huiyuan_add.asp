	...
	%>
      ...
      <tr>
        ...
        <td class="category">
		  ...
		  %>
		    <option value="<%=rs_login("username")%>"<%if rs_login("username")=session("redboy_username") then%> selected="selected"<%end if%>><%=rs_login("username")%> (<%=rs_login("bianhao")%>)</option>
		  <%
		    rs_login.movenext
		  loop
		  %>
