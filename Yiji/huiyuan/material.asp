...
<table align="center" cellpadding="4" cellspacing="1" class="toptable grid" border="1">
  <tr align="center">
	...
	<%if fla37="1" or session("redboy_id")="1" then%>
    <td class="category" width="8%">修改</td>
	<%end if%>
	<%if fla38="1" or session("redboy_id")="1" then%>
    <td class="category" width="5%">删除</td>
	<%end if%>
	<%if fla17="1" or session("redboy_id")="1" then%>
	<td class="category" width="15%">积分兑换礼物</td>
	<%end if%>
  </tr>
  <%
  ...
  <tr onMouseOver="this.className='highlight'" onMouseOut="this.className=''" <%if request("form")<>"" then%>onDblClick="window.opener.document.<%=request.querystring("form")%>.<%=request.querystring("field")%>.value='<%=rs_huiyuan("id")%>';window.opener.document.<%=request.querystring("form")%>.<%=request.querystring("field2")%>.value='<%=rs_huiyuan("username")%>';window.opener.document.<%=request.querystring("form")%>.<%=request.querystring("field3")%>.value='<%=rs_huiyuan("card")%>';window.close();"<%else%>onDblClick="javascript:var win=window.open('huiyuan_show.asp?id=<%=rs_huiyuan("id")%>','会员详细信息','width=853,height=470,top=176,left=161,toolbar=no,location=no,directories=no,status=no,menubar=no,resizable=no,scrollbars=yes'); win.focus()"<%end if%>>
    ...
    <%if fla37="1" or session("redboy_id")="1" then%>
	<td align="center"><a href="huiyuan_modi.asp?form=<%=request("form")%>&field=<%=request("field")%>&field2=<%=request("field2")%>&field3=<%=request("field3")%>&id=<%=rs_huiyuan("id")%>&page=<%=currentpage%>&zu=<%=nowzu%>&keyword=<%=nowkeyword%>&order1=<%=request("order1")%>&order2=<%=request("order2")%>&order3=<%=request("order3")%>&order4=<%=request("order4")%>&order5=<%=request("order5")%>&order6=<%=request("order6")%>&order7=<%=request("order7")%>&order8=<%=request("order8")%>&order9=<%=request("order9")%>&order10=<%=request("order10")%>&order11=<%=request("order11")%>&order12=<%=request("order12")%>&order13=<%=request("order13")%>&order14=<%=request("order14")%>&order15=<%=request("order15")%>"><img src="../images/res.gif" border="0" hspace="2" align="absmiddle">修改</a></td>
	<%end if%>
	<%if fla38="1" or session("redboy_id")="1" then%>
    <td align="center"><input type="checkbox" name="ID" value="<%=rs_huiyuan("id")%>" style="border:0"></td>
	<%end if%>
	<%if fla17="1" or session("redboy_id")="1" then%>
	<td align="center"><input type="button" value="积分兑换礼物" onClick="javascript:window.open('huiyuan_duihuan.asp?id=<%=rs_huiyuan("id")%>','积分兑换礼物','width=853,height=470,top=176,left=161,toolbar=no,location=no,directories=no,status=no,menubar=no,resizable=no,scrollbars=yes')" class="button"></td>
	<%end if%>
  </tr>
  <%
    rs_huiyuan.movenext
