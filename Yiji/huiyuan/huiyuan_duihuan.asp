...
sql="insert into sell(id_produit,bigclass,smallclass,title,huohao,id_ku,ku,shulian,guige,id_login,login,type,selldate,price,price2,id_huiyuan,photo,bianhao) values("&rs_produit("id")&",'"&rs_bigclass(0)&"','"&smallclass&"','"&rs_produit("title")&"','"&nowhuohao&"',"&rs_ku("id")&",'"&rs_ku("ku")&"',"&nowshulian&",'"&rs_produit("guige")&"',"&nowid_login&",'"&rs_login("username")&"',3,#"&date()&"#,"&nowprice&","&rs_produit("price2")&","&nowhuiyuan&",'"&rs_produit("photo")&"','"&nowbianhao&"')"
conn.execute(sql)
end if
next
...
sql="insert into sell(shulian,id_login,login,type,selldate,price,price2,id_huiyuan,bianhao,zu) values("&totalshulian&","&nowid_login&",'"&rs_login("username")&"',3,#"&date()&"#,"&totalprice&","&totalprice2&","&nowhuiyuan&",'"&nowbianhao&"',true)"
conn.execute(sql)
%>
<script language="javascript">
alert("兑换礼物成功！");
...
<td width="100%" background="../images/r_0.gif">
  <table cellpadding="0" cellspacing="0" width="100%">
    <tr>
      <td>&nbsp;<%=rs_huiyuan("username")%> 兑换礼物 (共有 <%=rs_huiyuan("jifen")%> 积分)(带*号的为必填项)</td>
	  <td align="right">&nbsp;</td>
    </tr>
  </table>
</td>
          ...
          <table cellpadding="0" cellspacing="0" width="100%" border=0>
		    ...
		    <tr id="cailiaohan<%=x%>"<%if x<>1 then%> style="display:none;"<%end if%>>
			  <td>
				...
				积分: <input type="text" name="price<%=x%>" style="width:40px" onKeyUp="this.value=this.value.replace(/[^\d.]/g,'')"  onafterpaste="this.value=this.value.replace(/[^\d.]/g,'')">
				<%
