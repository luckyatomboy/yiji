...
<BODY>
<%
if fla68="0" and session("redboy_id")<>"1" then
%>
...
<%
sql="select * from money_smallclass where id="&request("id")
...
<table align="center" cellpadding="4" cellspacing="1" class="toptable grid" border="1">
  <tr>
	...
	%>
	  <option value="<%=rs_bigclass("id")%>"<%if rs_bigclass("id")=rs("id_bigclass") then%> selected="selected"<%end if%>><%=rs_bigclass("bigclass")%></option>
	<%
  ...
  <tr>
    ...
    <td class="category">
        <input type="text" name="smallclass" style="width:300px" value="<%=rs("smallclass")%>">
...
sql="select * from money_smallclass where smallclass='"&nowsmallclass&"' and id<>"&request("id")
set rs=conn.execute(sql)
...
sql="update money_smallclass set id_bigclass="&nowbigclass&",smallclass='"&nowsmallclass&"' where id="&request("id")
conn.execute(sql)
%>
<script language="javascript">
alert("小类修改成功！")
