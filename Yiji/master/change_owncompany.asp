...
<%
  sql="select * from locktable where tablename='owncompany' and combinedkey='"&request("company")&"'"
...
<%else
    sql="insert into locktable(tablename,combinedkey,status,username,locktime) values('owncompany','"&request("company")&"','E','"&session("redboy_username")&"',#"&now()&"#)"  
    conn.execute(sql)
end if
%>
      ...
      <tr>
        ...
        <td class="category">
		  ...
		  <input type="hidden" name="hid1" value="ok">
			<input type="button" value=" 放弃修改返回 " onClick="if (confirm('确定要放弃修改吗？')) {window.open('delete_lock_table.asp?tablename=owncompany&combinedkey=<%=request("company")%>'); window.location.href='master.asp';}" class="button">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			<%
			if fla7="0" and session("redboy_id")<>"1" then
			else
			%>			
			<input type="button" value=" 删除 " onClick="if (confirm('确定要删除该公司吗？')) {window.open('delete_owncompany.asp?company=<%=request("company")%>'); window.location.href='master.asp';}" class="button"></td>
			<%end if%>	
      </tr>	    
</table>
</td>
...
sql="delete from locktable where tablename='owncompany' and combinedkey="&request("company")
conn.execute(sql)
