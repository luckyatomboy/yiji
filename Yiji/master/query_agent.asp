...
<form name="form1">
  ...
  %>
  <tr align="center">
  <td align="center"><%=rs_agent("company")%></td>	  
  <td align="center"><%=rs_agent("address")%></td>
  <td align="center"><%=rs_agent("tel")%></td>
  <td align="center"><%=rs_agent("fax")%></td>
  <td align="center"><%=rs_agent("email")%></td>
	<%if fla14="1" or session("redboy_id")="1" then%>
    <td align="center">
    	<a href="change_agent.asp?form=<%=request("form")%>&company=<%=rs_agent("company")%>&keyword=<%=nowkeyword%>"><img src="../images/res.gif" border="0" hspace="2" align="absmiddle">修改</a></td>
	<%end if%>
  </tr>
  <%
  	'end if
