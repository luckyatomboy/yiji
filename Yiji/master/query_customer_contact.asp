...
<%
  if nowkeyword<>"" then
  	sql="select * from customercontact where customername like '%"&nowkeyword&"%' order by customername,contact ASC"
	else
		sql="select * from customercontact order by customername,contact ASC"
  end if
%>
...
<form name="form1">
  ...
  <tr align="center">
	...
	<td class="category" width="100" height="30">QQ</td>
  ...
  rs_customer.open sql,conn,1,1
  if not rs_customer.eof then
  do while rs_customer.eof=false
  %>
  <tr align="center">
  <td align="center"><%=rs_customer("customername")%></td>	  
  <td align="center"><%=rs_customer("contact")%></td>
  <td align="center"><%=rs_customer("position")%></td>
  <td align="center"><%=rs_customer("tel")%></td>
  <td align="center"><%=rs_customer("fax")%></td>
  <td align="center"><%=rs_customer("email")%></td>
  <td align="center"><%=rs_customer("wechat")%></td>
  <td align="center"><%=rs_customer("qq")%></td>
	<%if fla14="1" or session("redboy_id")="1" then%>
    <td align="center">
    	<a href="change_customer_contact.asp?form=<%=request("form")%>&customername=<%=rs_customer("customername")%>&contact=<%=rs_customer("contact")%>&keyword=<%=nowkeyword%>"><img src="../images/res.gif" border="0" hspace="2" align="absmiddle">修改</a></td>
	<%end if%>
  </tr>
  <%
  	'end if
