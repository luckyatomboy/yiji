      ...
      <tr>
        ...
        <td class="category">
		  ...
		  <input type="hidden" name="hid1" value="ok">
			<input type="button" value=" 放弃修改返回 " onClick="if (confirm('确定要放弃修改吗？')) {window.open('delete_lock_table.asp?tablename=piwen&combinedkey=<%=request("company")%>'); window.history.go(-2);}" class="button">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			<%
			if fla7="0" and session("redboy_id")<>"1" then
			else
			%>			
			<input type="button" value=" 删除 " onClick="if (confirm('确定要删除该进口批文公司吗？')) {window.open('delete_piwen.asp?company=<%=request("company")%>')}" class="button"></td>
			<%end if%>	
      </tr>	    
</table>
</td>
