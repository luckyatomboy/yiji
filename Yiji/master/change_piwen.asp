...
<%
  sql="select * from locktable where tablename='piwen' and combinedkey='"&request("company")&"'"
...
<%else
    sql="insert into locktable(tablename,combinedkey,status,username,locktime) values('piwen','"&request("company")&"','E','"&session("redboy_username")&"',#"&now()&"#)"  
    conn.execute(sql)
end if
%>
...
sql="select * from piwen where company='"&request("company")&"'"
...
<table align="center" cellpadding="4" cellspacing="1" class="toptable grid" border="1">
  <form name="form2">
      ...
      <tr>
        <td width="25%" height="30" align="right">进口批文公司名称：</td>
      ...
      <tr>
        ...
        <td class="category">
		  ...
		  <input type="hidden" name="hid1" value="ok">
			<input type="button" value=" 放弃修改返回 " onClick="if (confirm('确定要放弃修改吗？')) {window.open('delete_lock_table.asp?tablename=piwen&combinedkey=<%=request("company")%>'); window.location.href='master.asp';}" class="button">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
			<%
			if fla7="0" and session("redboy_id")<>"1" then
			else
			%>			
			<input type="button" value=" 删除 " onClick="if (confirm('确定要删除该进口批文公司吗？')) {window.open('delete_piwen.asp?company=<%=request("company")%>'); window.location.href='master.asp';}" class="button"></td>
			<%end if%>	
      </tr>	    
</table>
</td>
...
<script language="javascript">
alert("进口批文公司资料修改成功！")
window.location.href="query_piwen.asp?form=<%=request("form")%>&keyword=<%=nowkeyword%>"
</script> 
<%
end if
%>
