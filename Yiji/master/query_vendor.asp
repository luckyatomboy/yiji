...
<%
  if nowkeyword<>"" then
  	sql="select * from vendor where vendorname like '%"&nowkeyword&"%' order by vendorname ASC"
	else
		sql="select * from vendor order by vendorname ASC"
  end if
%>
...
<form name="form1">
  ...
  <tr align="center">
	...
	<td class="category" width="100" height="30">国别</td>
	<td class="category" width="100" height="30">厂号1</td>
	<td class="category" width="100" height="30">厂号2</td>
  ...
  <td align="center"><%=rs_vendor("country")%></td>
  <td align="center"><%=rs_vendor("plant1")%></td>
  <td align="center"><%=rs_vendor("plant2")%></td>
  ...
  <td align="center"><%=rs_vendor("term1")%></td>
	<%if fla14="1" or session("redboy_id")="1" then%>
    <td align="center">
    	<a href="change_vendor.asp?form=<%=request("form")%>&vendorname=<%=rs_vendor("vendorname")%>&keyword=<%=nowkeyword%>"><img src="../images/res.gif" border="0" hspace="2" align="absmiddle">修改</a></td>
	<%end if%>
  </tr>
  <%
  	'end if
