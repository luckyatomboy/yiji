...
<%
sql="select * from vendorcontact where vendorname='"&request("vendorname")&"' and contact='"&request("contact")&"'"
      ...
      <tr>
        ...
        <td class="category">
		  ...
		  <input type="hidden" name="hid1" value="ok">
			...
			%>			
			<input type="button" value=" 删除 " onClick="if (confirm('确定要删除该供应商联系人吗？')) {window.open('delete_vendor_contact.asp?vendorname=<%=request("vendorname")%>&contact=<%=request("contact")%>')}" class="button"></td>
			<%end if%>			
      </tr>	    
</table>
</td>
...
<script language="javascript">
alert("客户联系人资料修改成功！")
window.location.href="query_vendor_contact.asp?form=<%=request("form")%>&keyword=<%=nowkeyword%>"
</script> 
<%
end if
%>
