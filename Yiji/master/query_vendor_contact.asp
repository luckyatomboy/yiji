...
<%
  if nowkeyword<>"" then
  	sql="select * from vendorcontact where vendorname like '%"&nowkeyword&"%' order by vendorname,contact ASC"
	else
		sql="select * from vendorcontact order by vendorname,contact ASC"
  end if
%>
...
<form name="form1">
  ...
  <tr align="center">
	<td class="category" width="150" height="30">供应商名称</td>
	<td class="category" width="100" height="30">联系人</td>
	<td class="category" width="100" height="30">职位</td>
	...
	<td class="category" width="100" height="30">微信</td>
	<td class="category" width="100" height="30">Skype</td>
  ...
  rs_vendor.open sql,conn,1,1
  if not rs_vendor.eof then
  do while rs_vendor.eof=false
  %>
  <tr align="center">
  <td align="center"><%=rs_vendor("vendorname")%></td>	  
  <td align="center"><%=rs_vendor("contact")%></td>
  <td align="center"><%=rs_vendor("position")%></td>
  <td align="center"><%=rs_vendor("tel")%></td>
  <td align="center"><%=rs_vendor("fax")%></td>
  <td align="center"><%=rs_vendor("email")%></td>
  <td align="center"><%=rs_vendor("wechat")%></td>
  <td align="center"><%=rs_vendor("skype")%></td>
	<%if fla14="1" or session("redboy_id")="1" then%>
    <td align="center">
    	<a href="change_vendor_contact.asp?form=<%=request("form")%>&vendorname=<%=rs_vendor("vendorname")%>&contact=<%=rs_vendor("contact")%>&keyword=<%=nowkeyword%>"><img src="../images/res.gif" border="0" hspace="2" align="absmiddle">修改</a></td>
	<%end if%>
  </tr>
  <%
  	'end if
