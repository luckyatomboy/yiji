...
<%
  if nowkeyword<>"" then
  	sql="select * from piwen where company like '%"&nowkeyword&"%' order by company ASC"
	else
		sql="select * from piwen order by company ASC"
  end if
%>
...
<form name="form1">
  ...
  <%
  set rs_piwen =server.createobject("ADODB.RecordSet")	
  rs_piwen.open sql,conn,1,3
  if not rs_piwen.eof then
  do while rs_piwen.eof=false
  %>
  <tr align="center">
  <td align="center"><%=rs_piwen("company")%></td>	  
  <td align="center"><%=rs_piwen("address")%></td>
  <td align="center"><%=rs_piwen("tel")%></td>
  <td align="center"><%=rs_piwen("fax")%></td>
  <td align="center"><%=rs_piwen("email")%></td>
	<%if fla14="1" or session("redboy_id")="1" then%>
    <td align="center">
    	<a href="change_piwen.asp?form=<%=request("form")%>&company=<%=rs_piwen("company")%>&keyword=<%=nowkeyword%>"><img src="../images/res.gif" border="0" hspace="2" align="absmiddle">修改</a></td>
	<%end if%>
  </tr>
  <%
  	'end if
  ...
  %>
  <%
  if rs_piwen.recordcount>0 then 
  %> 
</table></td></tr>
<%end if%> 
</form>   
