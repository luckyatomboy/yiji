...
if request("twodocumentready")<>"" then
	rs("twodocumentreadydate")=request("twodocumentready")
else
	rs("twodocumentreadydate")=null
end if
'第六行'
...
if request("planinsurance")<>"" then
  rs("planinsurancedate")=request("planinsurance")
else
  rs("planinsurancedate")=null
end if
if request("supinsurance")<>"" then
  rs("supinsurancedate")=request("supinsurance")
else
  rs("supinsurancedate")=null
end if
if request("insurancepayment")<>"" then
  rs("insurancepaymentdate")=request("insurancepayment")
else
  rs("insurancepaymentdate")=null
end if
...
if request("documentarrival")<>"" then
  rs("documentarrivaldate")=request("documentarrival")
else
  rs("documentarrivaldate")=null
end if
if request("planretirebill")<>"" then
  rs("planretirebilldate")=request("planretirebill")
else
  rs("planretirebilldate")=null
end if
if request("internalretirebill")<>"" then
  rs("internalretirebilldate")=request("internalretirebill")
else
  rs("internalretirebilldate")=null
end if
if request("externalretirebill")<>"" then
  rs("externalretirebilldate")=request("externalretirebill")
else
  rs("externalretirebilldate")=null
end if
'第十四行'
...
if request("returnwarranty")<>"" then
...
<!--	  <form name="form1">	-->
      ...
      <tr>	  
        ...
        <td class="category">
		  		...
		  		&nbsp;&nbsp;&nbsp;&nbsp;进口批文
					...
					&nbsp;&nbsp;&nbsp;两证齐备日期
		  		<input name="twodocumentready" id="twodocumentready" <%if rs("twodocumentreadydate")<>"" then%>value=<%=FormatDateTime(rs("twodocumentreadydate"),2)%><%end if%> class="datepicker"  style="width:80px">										 		
				</td>
      </tr>    
      <tr>	  
      ...
      <tr>	  
        ...
        <td class="category">
		  		<input name="planinsurance" id="planinsurance" class="datepicker" style="width:80px" <%if rs("planinsurancedate")<>"" then%>value=<%=FormatDateTime(rs("planinsurancedate"),2)%><%end if%>>
		  		&nbsp;&nbsp;&nbsp;&nbsp;补保日
		  		<input name="supinsurance" id="supinsurance" class="datepicker"  style="width:80px" <%if rs("supinsurancedate")<>"" then%>value=<%=FormatDateTime(rs("supinsurancedate"),2)%><%end if%>>
		  		&nbsp;&nbsp;&nbsp;&nbsp;保费支付日
		  		<input name="insurancepayment" id="insurancepayment" class="datepicker" style="width:80px" <%if rs("insurancepaymentdate")<>"" then%>value=<%=FormatDateTime(rs("insurancepaymentdate"),2)%><%end if%>>	
		  		&nbsp;&nbsp;&nbsp;&nbsp;保单
      ...
      <tr>	  
        ...
        <td class="category">
		  		<input name="documentarrival" id="documentarrival" class="datepicker" style="width:80px" <%if rs("documentarrivaldate")<>"" then%>value=<%=FormatDateTime(rs("documentarrivaldate"),2)%><%end if%> >
		  		&nbsp;&nbsp;&nbsp;预计赎单日期
		  		<input name="planretirebill" id="planretirebill" class="datepicker" style="width:80px" <%if rs("planretirebilldate")<>"" then%>value=<%=FormatDateTime(rs("planretirebilldate"),2)%><%end if%> >	  		
		  		&nbsp;&nbsp;&nbsp;我司赎单日期
		  		<input name="internalretirebill" id="internalretirebill" class="datepicker" style="width:80px" <%if rs("internalretirebilldate")<>"" then%>value=<%=FormatDateTime(rs("internalretirebilldate"),2)%><%end if%> >		  		
		  		&nbsp;&nbsp;&nbsp;代理赎单日期
		  		<input name="externalretirebill" id="externalretirebill" class="datepicker" style="width:80px" <%if rs("externalretirebilldate")<>"" then%>value=<%=FormatDateTime(rs("externalretirebilldate"),2)%><%end if%> >		  		
				</td>
      </tr>       
      <tr>	  
      ...
      <tr>
		  ...
		  <input type="button" value=" 放弃修改返回 " onClick="if (confirm('确定要放弃修改吗？')) {window.open('../master/delete_lock_table.asp?tablename=shipment&combinedkey=<%=request("shipment")%>'); $(window).off('beforeunload'); window.opener=null; window.open('','_self'); window.close();}" class="button">&nbsp;&nbsp;&nbsp;&nbsp;     
      <%if fla7="0" and session("redboy_id")<>"1" then
      else
      %>      
      <input type="button" value=" 删除 " onClick="if (confirm('确定要删除该船期表吗？')) {window.open('delete_shipment.asp?shipmentnum=<%=request("shipment")%>'); $(window).off('beforeunload'); window.opener=null; window.open('','_self'); window.close();}" class="button">
      <%end if%>        
      </tr>  
      </tbody>  
</table>  
