...
<%
  ...
  elseif request("order12")<>"" then
    sql=sql&" order by remainqty "&request("order12")    
  end if
...
<form name="form1" action="produit_del.asp">
  ...
  <tr align="center">
	...
	<td class="category" width="80" height="30">
		<a href="?order12=<%if request("order12")="asc" then%>desc<%else%>asc<%end if%>&form=<%=request("form")%>" class="title">剩余库存<%if request("order12")="asc" then%><img src="../images/up2.gif" border="0" hspace="2" align="absmiddle"><%else%><img src="../images/down2.gif" border="0" hspace="2" align="absmiddle"><%end if%></a>	
  </td>  
