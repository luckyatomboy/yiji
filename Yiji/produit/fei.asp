...
<form name="form1" action="fei_del.asp">
  ...
  <tr align="center">
	...
	<%if fla27="1" or session("redboy_id")="1" then%>
	<td class="category">完成</td>
	<%end if%>
	<%if fla28="1" or session("redboy_id")="1" then%>
    <td class="category">删除</td>
	<%end if%>
  </tr>
  <%
  set rs_buy =server.createobject("ADODB.RecordSet")
  sql="select * from sell where type=1 and zu"
  if nowstartdate<>"" then
  ...
  if nowkeyword<>"" then
    sql=sql&" and (bianhao like '%"&nowkeyword&"%' or id_login in (select id from login where username like '%"&nowkeyword&"%'))"
  end if
  ...
  %>
  <tr onMouseOver="this.className='highlight'" onMouseOut="this.className=''" onDblClick="javascript:var win=window.open('print_fei.asp?bianhao=<%=rs_buy("bianhao")%>','详细信息','width=853,height=470,top=176,left=161,toolbar=no,location=no,directories=no,status=no,menubar=no,resizable=no,scrollbars=yes'); win.focus()">
  ...
  <%
    set rs_buy2=conn.execute("select * from sell where type=1 and zu=false and bianhao='"&rs_buy("bianhao")&"'")
  %>
