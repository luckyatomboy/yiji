...
<%
if fla18="0" and session("redboy_id")<>"1" then
%>
...
<form name="form1" action="sell_del.asp">
  ...
  <tr align="center">
	...
	<%if fla19="1" or session("redboy_id")="1" then%>
	<td class="category">完成</td>
	<%end if%>
	<%if fla20="1" or session("redboy_id")="1" then%>
    <td class="category">删除</td>
	<%end if%>
  </tr>
  <%
  set rs_buy =server.createobject("ADODB.RecordSet")
  sql="select * from sell where type=0 and zu"
  if nowstartdate<>"" then
  ...
  if nowkeyword<>"" then
    sql=sql&" and (batch  like '%"&nowkeyword&"%' or orderid  like '%"&nowkeyword&"%' or bianhao like '%"&nowkeyword&"%' or id_huiyuan in (select id from huiyuan where username like '%"&nowkeyword&"%') or id_login in (select id from login where username like '%"&nowkeyword&"%'))"
  end if
  ...
  %>
  <tr onMouseOver="this.className='highlight'" onMouseOut="this.className=''" onDblClick="javascript:var win=window.open('print_sell.asp?bianhao=<%=rs_buy("bianhao")%>','详细信息','width=853,height=470,top=176,left=161,toolbar=no,location=no,directories=no,status=no,menubar=no,resizable=no,scrollbars=yes'); win.focus()">
