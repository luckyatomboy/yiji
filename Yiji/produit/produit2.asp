...
<table align="center" cellpadding="4" cellspacing="1" class="toptable grid" border="1">
  ...
  %>
  <tr onMouseOver="this.className='highlight'" onMouseOut="this.className=''" <%if request("form")<>"" then%>onDblClick="window.opener.document.<%=request("form")%>.<%=request("field")%>.value='<%=rs_produit("huohao")%>';<%if request("field2")<>"" then%>window.opener.document.<%=request("form")%>.<%=request("field2")%>.value='<%=rs_produit("price")%>';<%end if%><%if request("field3")<>"" then%>window.opener.document.<%=request("form")%>.<%=request("field3")%>.value='<%=rs_produit("price")%>';<%end if%><%if request("span1")<>"" then%>window.opener.document.all.<%=request("span1")%>.innerHTML='目前所有仓库数量：<%=nowshulian%>，<%=left(nowtext,len(nowtext)-1)%>';<%end if%>window.close();"<%else%>onDblClick="javascript:var win=window.open('produit_show.asp?id=<%=rs_produit("id")%>','产品详细信息','width=895,height=503,top=142,left=115,toolbar=no,location=no,directories=no,status=no,menubar=no,resizable=no,scrollbars=yes'); win.focus()"<%end if%>>
