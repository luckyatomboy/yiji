...
<%
if fla22="0" and session("redboy_id")<>"1" then
%>
...
<form name="form1" action="back_del.asp">
  ...
  <tr align="center">
	...
	<td class="category">
	  <a href="?order9=<%if request("order9")="asc" then%>desc<%else%>asc<%end if%>&page=<%=currentpage%>&startdate=<%=nowstartdate%>&enddate=<%=nowenddate%>&keyword=<%=nowkeyword%>" class="title">退货金额<%if request("order9")="asc" then%><img src="../images/up2.gif" border="0" hspace="2" align="absmiddle"><%else%><img src="../images/down2.gif" border="0" hspace="2" align="absmiddle"><%end if%></a>
	</td>
	<td class="category">
	...
	<%if fla23="1" or session("redboy_id")="1" then%>
	<td class="category">完成</td>
	<%end if%>
  ...
  sql="select * from buy where type=1 and zu"
  if nowstartdate<>"" then
  ...
  %>
  <tr onMouseOver="this.className='highlight'" onMouseOut="this.className=''" onDblClick="javascript:var win=window.open('print_back.asp?bianhao=<%=rs_buy("bianhao")%>','详细信息','width=853,height=470,top=176,left=161,toolbar=no,location=no,directories=no,status=no,menubar=no,resizable=no,scrollbars=yes'); win.focus()">
	...
	<%if fla23="1" or session("redboy_id")="1" then%>
	<td align="center"><%if rs_buy("isok") then%><a href="?bianhao=<%=rs_buy("bianhao")%>&isok=false&page=<%=currentpage%>&startdate=<%=nowstartdate%>&enddate=<%=nowenddate%>&keyword=<%=nowkeyword%>&order1=<%=request("order1")%>&order2=<%=request("order2")%>&order3=<%=request("order3")%>&order4=<%=request("order4")%>&order5=<%=request("order5")%>&order6=<%=request("order6")%>&order7=<%=request("order7")%>&order8=<%=request("order8")%>&order9=<%=request("order9")%>&order10=<%=request("order10")%>&order11=<%=request("order11")%>&order12=<%=request("order12")%>&order13=<%=request("order13")%>&order14=<%=request("order14")%>&order15=<%=request("order15")%>"><img src="../images/check.gif" border="0" alt="点击设为没完成"></a><%else%><a href="?bianhao=<%=rs_buy("bianhao")%>&isok=true&page=<%=currentpage%>&startdate=<%=nowstartdate%>&enddate=<%=nowenddate%>&keyword=<%=nowkeyword%>&order1=<%=request("order1")%>&order2=<%=request("order2")%>&order3=<%=request("order3")%>&order4=<%=request("order4")%>&order5=<%=request("order5")%>&order6=<%=request("order6")%>&order7=<%=request("order7")%>&order8=<%=request("order8")%>&order9=<%=request("order9")%>&order10=<%=request("order10")%>&order11=<%=request("order11")%>&order12=<%=request("order12")%>&order13=<%=request("order13")%>&order14=<%=request("order14")%>&order15=<%=request("order15")%>"><img src="../images/nomoney.gif" border="0" alt="点击设为已完成"></a><%end if%></td>
	<%end if%>
	<%if fla24="1" or session("redboy_id")="1" then%>
  ...
  <%
    set rs_buy2=conn.execute("select * from buy where type=1 and zu=false and bianhao='"&rs_buy("bianhao")&"'")
  %>
