...
<BODY>
<%
if fla5="0" and session("redboy_id")<>"1" then
%>
...
sql="insert into sell(id_produit,bigclass,smallclass,title,huohao,id_ku,ku,shulian,guige,id_login,login,type,selldate,price,photo,id_gys,gys,bianhao) values("&rs_produit("id")&",'"&rs_bigclass(0)&"','"&smallclass&"','"&rs_produit("title")&"','"&nowhuohao&"',"&rs_ku("id")&",'"&rs_ku("ku")&"',"&nowshulian&",'"&rs_produit("guige")&"',"&nowid_login&",'"&rs_login("username")&"',2,#"&date()&"#,"&nowprice&",'"&rs_produit("photo")&"',"&nowid_gys&",'"&nowgys&"','"&nowbianhao&"')"
conn.execute(sql)
end if
next
sql="insert into sell(shulian,id_login,login,type,selldate,price,id_gys,gys,bianhao,zu) values("&totalshulian&","&nowid_login&",'"&rs_login("username")&"',2,#"&date()&"#,"&totalprice&","&nowid_gys&",'"&nowgys&"','"&nowbianhao&"',true)"
conn.execute(sql)
%>
<script language="javascript">
alert("操作成功！")
...
<table align="center" cellpadding="4" cellspacing="1" class="toptable grid" border="1">
        ...
        <td width="80%" class="category">
          <table cellpadding="0" cellspacing="0" width="100%" border=0>
		    ...
		    <tr id="cailiaohan<%=x%>"<%if x<>1 then%> style="display:none;"<%end if%>>
			  <td>
			    <input name="huohao<%=x%>" readonly onClick="JavaScript:window.open('produit1.asp?form=form1&field=huohao<%=x%>&field2=price<%=x%>&span1=showshulian<%=x%>','','directorys=no,toolbar=no,status=no,menubar=no,scrollbars=yes,resizable=no,width=853,height=470,top=176,left=161');" style="width:80px" value="单击选择产品">
				...
				单价: <input type="text" name="price<%=x%>" style="width:40px" onKeyUp="value=value.replace(/[^\d.]/g,'')" onbeforepaste="clipboardData.setData('text',clipboardData.getData('text').replace(/[^\d.]/g,''))">
				<%
      ...
      <tr>
	    <td height="30">&nbsp;</td>
        <td class="category">
		  <input type="submit" value=" 确认退回 " onClick="return check1()" class="button">
