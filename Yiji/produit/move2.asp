...
if request("isok")<>"" then
  conn.execute("update kcpd set isok="&request("isok")&" where bianhao='"&request("bianhao")&"'")
end if
...
<form name="form1" action="move2_del.asp">
  ...
  <tr align="center">
	...
	<td class="category">
	  <a href="?order6=<%if request("order6")="asc" then%>desc<%else%>asc<%end if%>&page=<%=currentpage%>&startdate=<%=nowstartdate%>&enddate=<%=nowenddate%>&ku=<%=nowku%>&keyword=<%=nowkeyword%>" class="title">类别<%if request("order6")="asc" then%><img src="../images/up2.gif" border="0" hspace="2" align="absmiddle"><%else%><img src="../images/down2.gif" border="0" hspace="2" align="absmiddle"><%end if%></a></td>
	<td class="category">
  ...
  sql="select * from kcpd where type=0"
  ...
  if nowkeyword<>"" then
    sql=sql&" and (bianhao = '"&nowkeyword&"' or id_login in (select id from login where username like '%"&nowkeyword&"%') or bianhao in (select bianhao from kcpd where type=0 and (title like '%"&nowkeyword&"%' or huohao like '%"&nowkeyword&"%')))"
  end if
  ...
  elseif request("order6")<>"" then
    sql=sql&" order by lx "&request("order6")
  ...
  <tr onMouseOver="this.className='highlight'" onMouseOut="this.className=''">
	...
	<%end if%>
	<td align="center"><img src="../images/res.gif" border="0" hspace="2" align="absmiddle" style="cursor:hand;" onClick="javascript:var win=window.open('move_modi2.asp?id=<%=rs_buy("id")%>','修改备注','width=853,height=470,top=176,left=161,toolbar=no,location=no,directories=no,status=no,menubar=no,resizable=no,scrollbars=yes'); win.focus()"></td>
	<%if fla4="1" or session("redboy_id")="1" then%>
