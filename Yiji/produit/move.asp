...
if request("isok")<>"" then
  conn.execute("update diaobo set isok="&request("isok")&" where bianhao='"&request("bianhao")&"'")
end if
...
<form name="form1" action="move_del.asp">
  ...
  <tr align="center">
	...
	<td class="category" height="30">
	  <a href="?order2=<%if request("order2")="asc" then%>desc<%else%>asc<%end if%>&page=<%=currentpage%>&startdate=<%=nowstartdate%>&enddate=<%=nowenddate%>&ku=<%=nowku%>&keyword=<%=nowkeyword%>" class="title">货号<%if request("order2")="asc" then%><img src="../images/up2.gif" border="0" hspace="2" align="absmiddle"><%else%><img src="../images/down2.gif" border="0" hspace="2" align="absmiddle"><%end if%></a>	
	</td>
	<td class="category" height="30">
	  <a href="?order3=<%if request("order3")="asc" then%>desc<%else%>asc<%end if%>&page=<%=currentpage%>&startdate=<%=nowstartdate%>&enddate=<%=nowenddate%>&ku=<%=nowku%>&keyword=<%=nowkeyword%>" class="title">产品名称<%if request("order3")="asc" then%><img src="../images/up2.gif" border="0" hspace="2" align="absmiddle"><%else%><img src="../images/down2.gif" border="0" hspace="2" align="absmiddle"><%end if%></a>	
	</td>
	<td class="category">
	  <a href="?order4=<%if request("order4")="asc" then%>desc<%else%>asc<%end if%>&page=<%=currentpage%>&startdate=<%=nowstartdate%>&enddate=<%=nowenddate%>&ku=<%=nowku%>&keyword=<%=nowkeyword%>" class="title">调拨时间<%if request("order4")="asc" then%><img src="../images/up2.gif" border="0" hspace="2" align="absmiddle"><%else%><img src="../images/down2.gif" border="0" hspace="2" align="absmiddle"><%end if%></a>
    </td>
	<td class="category">
	...
	<td class="category">
	  <a href="?order6=<%if request("order6")="asc" then%>desc<%else%>asc<%end if%>&page=<%=currentpage%>&startdate=<%=nowstartdate%>&enddate=<%=nowenddate%>&ku=<%=nowku%>&keyword=<%=nowkeyword%>" class="title">出货仓库<%if request("order6")="asc" then%><img src="../images/up2.gif" border="0" hspace="2" align="absmiddle"><%else%><img src="../images/down2.gif" border="0" hspace="2" align="absmiddle"><%end if%></a></td>
	<td class="category">
	  <a href="?order9=<%if request("order9")="asc" then%>desc<%else%>asc<%end if%>&page=<%=currentpage%>&startdate=<%=nowstartdate%>&enddate=<%=nowenddate%>&ku=<%=nowku%>&keyword=<%=nowkeyword%>" class="title">入货仓库<%if request("order9")="asc" then%><img src="../images/up2.gif" border="0" hspace="2" align="absmiddle"><%else%><img src="../images/down2.gif" border="0" hspace="2" align="absmiddle"><%end if%></a>
	</td>  
	<td class="category">
	...
	<%if fla3="1" or session("redboy_id")="1" then%>
	<td class="category">
	...
	<%if fla4="1" or session("redboy_id")="1" then%>
    <td class="category">删除</td>
	<%end if%>
  </tr>
  <%
  set rs_buy =server.createobject("ADODB.RecordSet")
  sql="select * from diaobo where type=0"
  if session("redboy_id")<>1 and lookbuysell="yes" then
    sql=sql&" and id_login="&session("redboy_id")
  end if
  if nowstartdate<>"" then
  ...
  if nowku<>"" then
	sql=sql&" and (id_ku_chu="&nowku&" or id_ku_ru="&nowku&")"
  end if 
  if nowkeyword<>"" then
    sql=sql&" and (bianhao = '"&nowkeyword&"' or id_login in (select id from login where username like '%"&nowkeyword&"%') or bianhao in (select bianhao from diaobo where type=0 and (title like '%"&nowkeyword&"%' or huohao like '%"&nowkeyword&"%')))"
  end if
  ...
  elseif request("order6")<>"" then
    sql=sql&" order by id_ku_chu "&request("order6")
  ...
  elseif request("order9")<>"" then
    sql=sql&" order by id_ku_ru "&request("order9")
  ...
  <tr onMouseOver="this.className='highlight'" onMouseOut="this.className=''">
	...
	<td align="center"><%if rs_produit.eof then%><%=rs_buy("huohao")%><%else%><%=rs_produit("huohao")%><%end if%></td>
	...
	<td align="center"><%=rs_buy("ku_chu")%></td>
	<td align="center"><%=rs_buy("ku_ru")%></td>
	<td align="center">
	  <%=formatnumber(rs_buy("shulian"),3)%>
	</td>
	<%if fla15="1" or session("redboy_id")="1" then%>
	<td align="center"><%if rs_buy("isok")=0 then%><a href="?bianhao=<%=rs_buy("bianhao")%>&isok=1&page=<%=currentpage%>&startdate=<%=nowstartdate%>&enddate=<%=nowenddate%>&ku=<%=nowku%>&keyword=<%=nowkeyword%>&order1=<%=request("order1")%>&order2=<%=request("order2")%>&order3=<%=request("order3")%>&order4=<%=request("order4")%>&order5=<%=request("order5")%>&order6=<%=request("order6")%>&order7=<%=request("order7")%>&order8=<%=request("order8")%>&order9=<%=request("order9")%>&order10=<%=request("order10")%>&order11=<%=request("order11")%>&order12=<%=request("order12")%>&order13=<%=request("order13")%>&order14=<%=request("order14")%>&order15=<%=request("order15")%>"><img src="../images/check.gif" border="0" alt="点击设为没完成"></a><%else%><a href="?bianhao=<%=rs_buy("bianhao")%>&isok=0&page=<%=currentpage%>&startdate=<%=nowstartdate%>&enddate=<%=nowenddate%>&ku=<%=nowku%>&keyword=<%=nowkeyword%>&order1=<%=request("order1")%>&order2=<%=request("order2")%>&order3=<%=request("order3")%>&order4=<%=request("order4")%>&order5=<%=request("order5")%>&order6=<%=request("order6")%>&order7=<%=request("order7")%>&order8=<%=request("order8")%>&order9=<%=request("order9")%>&order10=<%=request("order10")%>&order11=<%=request("order11")%>&order12=<%=request("order12")%>&order13=<%=request("order13")%>&order14=<%=request("order14")%>&order15=<%=request("order15")%>"><img src="../images/nomoney.gif" border="0" alt="点击设为已完成"></a><%end if%></td>
	<%end if%>
	<td align="center"><img src="../images/res.gif" border="0" hspace="2" align="absmiddle" style="cursor:hand;" onClick="javascript:var win=window.open('move_modi.asp?id=<%=rs_buy("id")%>','修改备注','width=853,height=470,top=176,left=161,toolbar=no,location=no,directories=no,status=no,menubar=no,resizable=no,scrollbars=yes'); win.focus()"></td>
	<%if fla4="1" or session("redboy_id")="1" then%>
