...
<form name="form1" action="produit_del.asp">
  ...
  <tr align="center">
	<td class="category" height="30">
	  <a href="?order2=<%if request("order2")="asc" then%>desc
	  	<%else%>asc<%end if%>
	  		&form=<%=request("form")%>&field=<%=request("field")%>&field2=<%=request("field2")%>&page=<%=currentpage%>&keyword=<%=nowkeyword%>&ku=<%=nowku%>&bigclass=<%=nowbigclass%>&smallclass=<%=nowsmallclass%>
	  		" class="title">货号<%if request("order2")="asc" then%><img src="../images/up2.gif" border="0" hspace="2" align="absmiddle"><%else%><img src="../images/down2.gif" border="0" hspace="2" align="absmiddle"><%end if%>
	  		</a>	
	</td>
	<td class="category">
  ...
  <tr onMouseOver="this.className='highlight'" onMouseOut="this.className=''" 
  	<%if request("form")<>"" then%>
  		onDblClick="window.opener.document.<%=request.querystring("form")%>.<%=request.querystring("field")%>.value='<%=rs_produit("huohao")%>';window.opener.document.all.showshulian.innerHTML='目前所有仓库数量：<%=nowshulian%>，<%=left(nowtext,len(nowtext)-1)%>';<%if request("field2")<>"" then%>window.opener.document.<%=request.querystring("form")%>.<%=request.querystring("field2")%>.value='<%=rs_produit("price")%>';<%end if%>window.close();"<%else%>onDblClick="javascript:var win=window.open('produit_show.asp?id=<%=rs_produit("id")%>','产品详细信息','width=853,height=470,top=176,left=161,toolbar=no,location=no,directories=no,status=no,menubar=no,resizable=no,scrollbars=yes'); win.focus()"<%end if%>>
