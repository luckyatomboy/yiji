...
<form name="form1" action="produit_del.asp">
  ...
  <tr align="center">    
    ...
    <td class="category">调拨</td>
    <td class="category">盘点</td>
	<%end if%>
  </tr>
  <%
  ...
  %>
  <tr onMouseOver="this.className='highlight'" onMouseOut="this.className=''" <%if request("form")<>"" then%>onDblClick="window.opener.document.<%=request.querystring("form")%>.<%=request.querystring("field")%>.value='<%=rs_produit("huohao")%>';window.opener.document.all.showshulian.innerHTML='目前所有仓库数量：<%=nowshulian%>，<%=left(nowtext,len(nowtext)-1)%>';<%if request("field2")<>"" then%>window.opener.document.<%=request.querystring("form")%>.<%=request.querystring("field2")%>.value='<%=rs_produit("price")%>';<%end if%>window.close();"<%else%>onDblClick="javascript:var win=window.open('produit_show.asp?id=<%=rs_produit("id")%>','产品详细信息','width=853,height=470,top=176,left=161,toolbar=no,location=no,directories=no,status=no,menubar=no,resizable=no,scrollbars=yes'); win.focus()"<%end if%>>
    ...
    <td align="center"><a href="produit_move2.asp?id=<%=rs_produit("id")%>&page=<%=currentpage%>&keyword=<%=nowkeyword%>&ku=<%=nowku%>&bigclass=<%=nowbigclass%>&smallclass=<%=nowsmallclass%>&order1=<%=request("order1")%>&order2=<%=request("order2")%>&order3=<%=request("order3")%>&order4=<%=request("order4")%>&order5=<%=request("order5")%>&order6=<%=request("order6")%>&order7=<%=request("order7")%>&order8=<%=request("order8")%>&order9=<%=request("order9")%>&order10=<%=request("order10")%>&order11=<%=request("order11")%>&order12=<%=request("order12")%>&order13=<%=request("order13")%>&order14=<%=request("order14")%>&order15=<%=request("order15")%>" <%if rs_produit("shulian")<=0 then%>onclick="alert('此仓库中数量为零，无法调出！');return false;"<%end if%>><img src="../images/Comment.gif" border="0" hspace="2" align="absmiddle">调出</a></td>
    <td align="center"><a href="produit_move3.asp?id=<%=rs_produit("id")%>&page=<%=currentpage%>&keyword=<%=nowkeyword%>&ku=<%=nowku%>&bigclass=<%=nowbigclass%>&smallclass=<%=nowsmallclass%>&order1=<%=request("order1")%>&order2=<%=request("order2")%>&order3=<%=request("order3")%>&order4=<%=request("order4")%>&order5=<%=request("order5")%>&order6=<%=request("order6")%>&order7=<%=request("order7")%>&order8=<%=request("order8")%>&order9=<%=request("order9")%>&order10=<%=request("order10")%>&order11=<%=request("order11")%>&order12=<%=request("order12")%>&order13=<%=request("order13")%>&order14=<%=request("order14")%>&order15=<%=request("order15")%>" <%if rs_produit("shulian")<=0 then%>onclick="alert('此仓库中数量为零，无法调出！');return false;"<%end if%>><img src="../images/Comment.gif" border="0" hspace="2" align="absmiddle">盘点</a></td>    	
	<%end if%>
  </tr>
  <%
  	'end if
...
%>
    <%if fla16="1" or session("redboy_id")="1" then%>
