...
nowkeyword=request("keyword")
  ...
  sql="select * from buy where type=0 and zu = false and isok"
...
do while count_buy.eof=false
	  ...
	  if nowkeyword<>"" then
        sql3=sql3&" and (batch  like '%"&nowkeyword&"%' or orderid  like '%"&nowkeyword&"%' or bianhao = '"&nowkeyword&"' or id_gys in (select id from gys where company like '%"&nowkeyword&"%') or id_login in (select id from login where username like '%"&nowkeyword&"%') or title like '%"&nowkeyword&"%' or huohao like '%"&nowkeyword&"%')"
  	  end if
...
<tr>
  ...
  <th>供应商</th>
  <th>数量</th>
  <%if session("redboy_id")="1" or fla87="1" then%>
  <th>金额</th>
  <%end if%>
</tr>
<%
