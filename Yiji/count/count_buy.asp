...
<%
if fla29="0" and session("redboy_id")<>"1" then
%>
...
if request("isok")<>"" then
  conn.execute("update buy set isok="&request("isok")&" where id="&request("id"))
end if
...
nowkeyword=request("keyword") 
  ...
  if nowkeyword<>"" then
    sql=sql&" and (batch  like '%"&nowkeyword&"%' or orderid  like '%"&nowkeyword&"%' or bianhao like '%"&nowkeyword&"%' or id_gys in (select id from gys where company like '%"&nowkeyword&"%') or id_login in (select id from login where username like '%"&nowkeyword&"%'))"
  end if
...
do while count_buy.eof=false
  nowprice2=nowprice2+count_buy("price2")	
  count_buy.movenext
loop
%>
...
<td width="100%" background="../images/r_0.gif">
  <table cellpadding="0" cellspacing="0" width="100%">
    <tr>
      <td>&nbsp;进货统计 <a href="excel_buy.asp?page=<%=currentpage%>&startdate=<%=nowstartdate%>&enddate=<%=nowenddate%>&ku=<%=nowku%>&keyword=<%=nowkeyword%>&order1=<%=request("order1")%>&order2=<%=request("order2")%>&order3=<%=request("order3")%>&order4=<%=request("order4")%>&order5=<%=request("order5")%>&order6=<%=request("order6")%>&order7=<%=request("order7")%>&order8=<%=request("order8")%>&order9=<%=request("order9")%>&order10=<%=request("order10")%>&order11=<%=request("order11")%>&order12=<%=request("order12")%>&order13=<%=request("order13")%>&order14=<%=request("order14")%>&order15=<%=request("order15")%>" target="_blank"><img src="../images/excel.jpg" border="0" align="absmiddle" alt="导出Excel表格"></a>
      	<a href="excel_buyall.asp?page=<%=currentpage%>&startdate=<%=nowstartdate%>&enddate=<%=nowenddate%>&ku=<%=nowku%>&keyword=<%=nowkeyword%>&order1=<%=request("order1")%>&order2=<%=request("order2")%>&order3=<%=request("order3")%>&order4=<%=request("order4")%>&order5=<%=request("order5")%>&order6=<%=request("order6")%>&order7=<%=request("order7")%>&order8=<%=request("order8")%>&order9=<%=request("order9")%>&order10=<%=request("order10")%>&order11=<%=request("order11")%>&order12=<%=request("order12")%>&order13=<%=request("order13")%>&order14=<%=request("order14")%>&order15=<%=request("order15")%>" target="_blank"><img src="../images/excel.jpg" border="0" align="absmiddle" alt="导出Excel表格(明细)"></a>
      	</td>
	  <td align="right">总进货金额：<b><%=formatnumber(nowprice2,3)%></b> 元&nbsp;</td>
    </tr>
  </table>
</td>
...
<table align="center" cellpadding="4" cellspacing="1" class="toptable grid" border="1">
  ...
  %>
  <tr onMouseOver="this.className='highlight'" onMouseOut="this.className=''" onDblClick="javascript:var win=window.open('../produit/print_buy.asp?bianhao=<%=rs_buy("bianhao")%>','详细信息','width=853,height=470,top=176,left=161,toolbar=no,location=no,directories=no,status=no,menubar=no,resizable=no,scrollbars=yes'); win.focus()">
  ...
  <%
    set rs_buy2=conn.execute("select * from buy where type=0 and zu=false and bianhao='"&rs_buy("bianhao")&"'")
  %>
