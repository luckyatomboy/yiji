...
nowkeyword=request("keyword")
  ...
  if nowku<>"" then
	sql=sql&" and bianhao in (select bianhao from sell where type=1 and id_ku="&nowku&" and zu=false)"
  end if  
  if nowkeyword<>"" then
    sql=sql&" and (bianhao = '"&nowkeyword&"' or id_login in (select id from login where username like '%"&nowkeyword&"%') or bianhao in (select bianhao from sell where type=1 and (title like '%"&nowkeyword&"%' or huohao like '%"&nowkeyword&"%')))"
  end if
...
<td align="center">
	  ...
	  if nowkeyword<>"" then
        sql=sql&" and (bianhao = '"&nowkeyword&"' or id_login in (select id from login where username like '%"&nowkeyword&"%') or title like '%"&nowkeyword&"%' or huohao like '%"&nowkeyword&"%')"
  	  end if
	  set rs_sum=conn.execute(sql)
	  %>
	  <%=formatnumber(rs_sum(0),3)%>
</td>
</tr>
<%
rs_buy.movenext
