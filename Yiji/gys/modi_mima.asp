...
<%
sql="select * from gys where id="&session("gys_id")
...
sql="select * from gys where id="&session("gys_id")&" and password='"&md5(pwd_old)&"'"
set rs=conn.execute(sql)
if rs.eof then
%>
<script language="javascript">
...
sql="update gys set password='"&md5(pwd_new)&"' where id="&request("id")
conn.execute(sql)
%>
<script language="javascript">
alert("密码修改成功！")
