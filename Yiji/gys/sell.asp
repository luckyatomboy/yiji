...
nowkeyword=request("keyword") 
  ...
  sql="select * from sell where type=0 and zu and isok and id_gys="&session("gys_id")
  if nowstartdate<>"" then
  ...
  sql2="select * from buy where type=1 and zu and isok and id_gys="&session("gys_id")
  if nowstartdate<>"" then
  ...
  if nowkeyword<>"" then
    sql2=sql2&" and (bianhao like '%"&nowkeyword&"%' or id_huiyuan in (select id from huiyuan where username like '%"&nowkeyword&"%') or id_login in (select id from login where username like '%"&nowkeyword&"%'))"
  end if  
...
<td width="100%" background="../images/r_0.gif">
  <table cellpadding="0" cellspacing="0" width="100%">
    <tr>
      <td>&nbsp;销售统计</td>
	  <td align="right">总销售额：<i><%=formatnumber(nowmoney,3)%></i> 元，顾客退货：<u><%=formatnumber(nowmoney2,3)%></u> 元，实际销售：<b><%=formatnumber(nowmoney-nowmoney2,3)%></b> 元，销售利润：<b><%=formatnumber(nowmoney-nowprice-nowmoney2+nowprice2,3)%></b> 元&nbsp;</td>
    </tr>
  </table>
</td>
...
<table align="center" cellpadding="4" cellspacing="1" class="toptable grid" border="1">
  <tr align="center">
	...
	<td class="category">
	  <a href="?order4=<%if request("order4")="asc" then%>desc<%else%>asc<%end if%>&page=<%=currentpage%>&startdate=<%=nowstartdate%>&enddate=<%=nowenddate%>&keyword=<%=nowkeyword%>" class="title">时间
	  <%if request("order4")="asc" then%><img src="../images/up2.gif" border="0" hspace="2" align="absmiddle"><%else%><img src="../images/down2.gif" border="0" hspace="2" align="absmiddle"><%end if%></a>
    </td>
	<td class="category">
	...
	<td class="category">
	  <a href="?order10=<%if request("order10")="asc" then%>desc<%else%>asc<%end if%>&page=<%=currentpage%>&startdate=<%=nowstartdate%>&enddate=<%=nowenddate%>&keyword=<%=nowkeyword%>" class="title">流向<%if request("order10")="asc" then%><img src="../images/up2.gif" border="0" hspace="2" align="absmiddle"><%else%><img src="../images/down2.gif" border="0" hspace="2" align="absmiddle"><%end if%></a>
	</td>
  </tr>
  <%
  ...
  <tr onMouseOver="this.className='highlight'" onMouseOut="this.className=''" onDblClick="javascript:var win=window.open('../produit/print_sell.asp?bianhao=<%=rs_buy("bianhao")%>','详细信息','width=853,height=470,top=176,left=161,toolbar=no,location=no,directories=no,status=no,menubar=no,resizable=no,scrollbars=yes'); win.focus()">
	...
	<td align="center">
	  ...
	  %>
	  <%if rs_huiyuan.eof=false then%><%=rs_huiyuan("username")%><%else%>&nbsp;<%end if%>	
	</td>
  </tr>
  <%
    ...
    <td colspan="16" class="category">
	  <table align="center" cellpadding="4" cellspacing="1" class="grid" border="1" width="100%">
	    ...
	    <tr>
		  ...
		  <td align="center"><%if rs_produit.eof then%><%=rs_buy2("guige")%><%else%><%=rs_produit("guige")%><%end if%></td>
